defmodule MatchmakerWeb.MatchSessionLiveTest do
  ...
  defp create_match_session(_) do
  ...
  describe "Index" do
    ...
    test "saves new match_session", %{conn: conn} do
      ...
      {:ok, _, html} =
        index_live
        |> form("#match_session-form", match_session: @create_attrs)
        |> render_submit()
    ...
    test "updates match_session in listing", %{conn: conn, match_session: match_session} do
      ...
      assert index_live |> element("#match_session-#{match_session.id} a", "Edit") |> render_click() =~
               "Edit Match session"
    ...
    test "deletes match_session in listing", %{conn: conn, match_session: match_session} do
      ...
      assert index_live |> element("#match_session-#{match_session.id} a", "Delete") |> render_click()
  ...
  describe "Show" do
    ...
    test "displays match_session", %{conn: conn, match_session: match_session} do
      {:ok, _show_live, html} = live(conn, Routes.match_session_show_path(conn, :show, match_session))

      assert html =~ "Show Match session"
    end

    test "updates match_session within modal", %{conn: conn, match_session: match_session} do
      {:ok, show_live, _html} = live(conn, Routes.match_session_show_path(conn, :show, match_session))
